...
function collectSources(entryFile, normalize) {
  ...
  function resolveImport(importPath, fromFileDir) {
    ...
    for (const c of candidates) {
      if (fs.existsSync(c)) return { filePath: c, importKey: importPath };
    }
...
async function main() {
  ...
  const target = '0x' + decoded[0].ipfs.toString('hex').slice(4); // sha256 part after 0x1220
  ...
  for (const normalize of [false, true]) {
    for (const useLit of [false, true]) {
      const fullSources = {};
      for (const entry of entries) Object.assign(fullSources, collectSources(entry, normalize));
      const input = {
        language: 'Solidity',
        sources: fullSources,
        settings: {
      ...
      console.log('normalize', normalize, 'useLit', useLit, 'hash', hash, 'match', hash.toLowerCase() === target.toLowerCase());
    }
  }
}
