...
export default function NFTLaunch() {
  ...
  const [form, setForm] = useState<FormState>(initialForm); const [busy, setBusy] = useState(""); const [result, setResult] = useState(""); const [deployStage, setDeployStage] = useState("");
  ...
  const deploy = async () => { if (!wallet.isConnected || !wallet.signer) { setDeployStage("正在连接钱包…"); await wallet.connectWallet(); setDeployStage(""); return; } if (!wallet.isBSC) { setDeployStage("正在切换到 BNB Smart Chain…"); await wallet.switchToBSC(); setDeployStage(""); return; } setBusy("deploy"); setDeployStage("正在准备 NFT 元数据…"); try { const prepared = await prepareNFTMetadata(form); setForm((prev) => ({ ...prev, baseURI: prepared.baseURI, metadataURI: prepared.metadataURI })); const res = await createNFTLaunch(wallet.signer, prepared, (stage) => setDeployStage(stage === "vanity" ? "正在计算 7777 靓号地址，请稍候…" : stage === "wallet" ? "请在钱包中确认 0.01 BNB 创建交易…" : "交易已提交，正在等待链上确认…")); setResult(res.collection); setDeployStage("创建成功，正在提交源码验证…"); if (res.collection) void queueNFTVerification(res.collection); showToast({ type: "success", message: "NFT 合集创建成功，源码验证已提交" }); } catch (e) { showToast({ type: "error", message: e instanceof Error ? e.message : "创建失败" }); } finally { setBusy(""); setDeployStage(""); } };
